r"""
This module is a collection of functions needed to run m5Cnet dataprep
"""
import os, sys
...
import helper
from constants import M5C_KMERS
import argparse
...
def parallel_preprocess_tx(eventalign_filepath: str, out_dir: str, n_processes: int, readcount_min: int,
    ...
    with open(out_paths['info'], 'w', encoding='utf-8') as f:
        f.write('transcript_id,transcript_position,start,end,n_reads,kmer\n')  # header
...
def preprocess_tx(tx_id: str, data_dict: Dict, n_neighbors: int, min_segment_count: int, out_paths: Dict,
    ...
    for _, events_per_read in data_dict.items():
        events_per_read = filter_events(events_per_read, n_neighbors, M5C_KMERS)
    ...
    with locks['json'], open(out_paths['json'], 'a', encoding='utf-8') as f:
        with locks['info'], open(out_paths['info'], 'a', encoding='utf-8') as g:
            for pos, dat in data.items():
                ...
                g.write('%s,%d,%d,%d,%d,%s\n' % (tx_id, pos, pos_start, pos_end, n_reads, list(dat.keys())[0]))

    with locks['log'], open(out_paths['log'], 'a', encoding='utf-8') as f:
        f.write(log_str + '\n')


def proprecess_input(inputs, outputs, threads, readcount_min, readcount_max, neighbors, min_segment_count, chunk_size):
    # input eventalign
    if os.path.lexists(os.path.expanduser(inputs)) is False:
        sys.exit("[ERROR]Can't get your input eventalign file, please check the PATH!\n")
    elif os.path.isfile(inputs):
        pass
    else:
        sys.exit("[ERROR]Please check your input eventalign file! ")
    sys.stderr.write("Input eventalign file is ready!\n")
    ...
    # readcount_min
    if isinstance(readcount_min, int):
        print("readcount_min : %d" % readcount_min)
    else:
        sys.stderr.write("[ERROR]readcount_min is not an INTEGER!\n")
    # readcount_max
    if isinstance(readcount_max, int):
        print("readcount_max : %d" % readcount_max)
    else:
        sys.stderr.write("[ERROR]readcount_max is not an INTEGER!\n")
    # neighbors
    if isinstance(neighbors, int):
        print("neighbors : %d" % neighbors)
    else:
        sys.stderr.write("[ERROR]Neighbors is not an INTEGER!\n")
    # min_segment_count
    if isinstance(min_segment_count, int):
        print("min_segment_count : %d" % min_segment_count)
    else:
        sys.stderr.write("[ERROR]min_segment_count is not an INTEGER!\n")
    # chunk_size
    if isinstance(chunk_size, int):
        print("chunk_size : %d" % chunk_size)
    else:
        sys.stderr.write("[ERROR]chunk_size is not an INTEGER!\n")

    parallel_index(inputs, chunk_size, outputs, threads)
    parallel_preprocess_tx(inputs, outputs, threads, readcount_min, readcount_max, neighbors,
                           min_segment_count, compress=False)


def main():
    parser = argparse.ArgumentParser(description=desc, epilog=epilog, formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument("-i", "--input_eventalign", required=1, type=str, help="input eventalign file path")
    parser.add_argument("-o", "--outputs", required=1, type=str, help="output path")
    parser.add_argument("-t", "--threads", default=1, type=int, help="number of cores to use [default:1]")
    parser.add_argument("-r1", "--readcount_min", default=30, type=int, help="Minimum read counts over each candidate m5C segment[default:30]")
    parser.add_argument("-r2", "--readcount_max", default=200000, type=int,
                        help="Maximum read counts per gene[default:200000]")
    parser.add_argument("-n", "--neighbors", default=1, type=int, help="The number of flanking positions to process[default:1]")
    parser.add_argument("-s", "--min_segment_count", default=20, type=int,
                        help="Minimum value of segment count[default:20]")
    parser.add_argument("-c", "--chunk_size", default=100000, type=int,
                        help="Argument for pandas read csv function on the eventalign input[default:100000]")

    o = parser.parse_args()
    proprecess_input(o.input_eventalign, o.outputs, o.threads, o.readcount_min, o.readcount_max, o.neighbors,
                     o.min_segment_count, o.chunk_size)


if __name__ == '__main__':
    desc = "Using 'Nanopolish eventalign' output to generate the feature of current intensity. \
    This step may use lots of time. You can use command 'python dataprep.py --help' see the specific usage of parameter."
