...
def run_inference(model: MILModel, dl: DataLoader, out_dir,device,save_per_batch,read_proba_threshold,num_iterations,n_processes):
    r'''
    ...
    with open(os.path.join(out_dir, "data.site_proba.csv"), 'a', encoding='utf-8') as f:
        with open(os.path.join(out_dir, "data.indiv_proba.csv"), 'a', encoding='utf-8') as g:
            with torch.no_grad():
                ...
                for it, data in enumerate(dl):
                    ...
                    features = model.get_read_representation({'X': features.to(device),
                                                              'kmer': kmers.to(device)})
                    ...
                    if (it + 1) % save_per_batch:
                        ...
                        mod_ratios = np.array([np.mean(x >= read_proba_threshold) for x in read_probs])
                        site_probs = calculate_site_proba(read_probs, num_iterations, 20, n_processes)
